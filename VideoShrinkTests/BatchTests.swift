...
@MainActor final class BatchTests: XCTestCase {
    ...
    func testRenderEveryScreenToAPng() async throws {
        ...
        // The one-video flow's screens take values rather than a model, and `SingleVideoFlow` is the
        // scroll view and the bar around them - so each is drawn inside the scroll view it actually
        // lives in. Without that the content is compressed into the frame and its text truncates,
        // which is the harness drawing itself.
        try draw("09-one-video-welcome", ShrinkWelcome(), into: directory, wrappedInScrollView: true)
        try draw("10-one-video-working",
                 ShrinkProgress(stage: .transcoding, progress: 0.42, fromCloud: false,
                                cancelling: false),
                 into: directory, wrappedInScrollView: true)
    ...
    private func drawLargeTextScreens(postRun: BatchFixture, source: VideoMetadata,
        ...
        try draw("L07-one-video-result", ShrinkResult(source: source, output: output, saved: false),
                 into: directory, dynamicTypeSize: large, wrappedInScrollView: true)
    }
    ...
    ///
    /// **A view handed a fixed frame and given more content than fits does not overflow it - SwiftUI
    /// compresses it, truncating Text with an ellipsis.** That is the second thing this method
    /// learned the hard way, and it is the more embarrassing one. The accessibility renders came
    /// back with "Your copy is ready. Gi…" and "294 MB less than the…", neither of which can happen
    /// on a device: neither of those Texts has a line limit, and the only three `lineLimit(1)` sites
    /// in the app are a pill, a brand lockup and the figure inside the progress orb. The pass
    /// designed to find text that does not fit was manufacturing text that does not fit.
    ///
    /// The screens differ in where they expect to be. Every batch screen owns a ScrollView and a
    /// bottom bar, so one is drawn on its own. The one-video screens own neither - `SingleVideoFlow`
    /// is the scroll view and the bar around them - so drawing one bare gives a view that is
    /// compressed rather than scrolled. `wrappedInScrollView` puts those four where they actually
    /// live. It cannot put them in `SingleVideoFlow`'s bar as well, so those pictures show the
    /// screen without its bar, which is the one thing they are honest about not having.
    @MainActor
