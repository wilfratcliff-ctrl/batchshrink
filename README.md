...
**The CI gate is blocked until 1 October 2026, and the honest status of `main` after commit `31b6245` is "unverified".** Every job stopped getting a runner: three jobs and a re-run each failed about nine seconds after creation, with no runner and no steps, while GitHub reported all systems operational and a throwaway Linux job could not start either. The account's own billing notice explains it - the plan's 2,000 Actions minutes for the month are used up, and they reset on 1 October. The pool is shared by every runner and macOS draws from it at ten minutes per minute, so the three-job workflow costs about 250 of those 2,000 per push. Nothing is being charged: a $0 budget blocks further use. `AGENT_LOOP.md` has the evidence, the arithmetic, the options, and what a round should do in the meantime.
