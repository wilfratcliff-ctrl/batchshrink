...
**The CI gate is blocked as of 2026-09-23, and the honest status of `main` is "unverified".** Every job stopped getting a runner: three jobs and a re-run each failed about nine seconds after creation with no runner and no steps, GitHub reported all systems operational, and a throwaway Linux job could not start either - so it is not a macOS-only fault. The account is on a free plan and this repository is private, where runner minutes come from a monthly allowance and macOS runners are billed at ten times the Linux rate. `AGENT_LOOP.md` has the evidence, the arithmetic and the options.
