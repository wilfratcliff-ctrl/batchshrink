...
- `scripts/sync-native-sources.mjs`: copies the shared Swift files and the privacy manifest into the module's ignored generated directory - 37 Swift files at the time of writing, the count the mirror check prints. It excludes the standalone `@main` app and standalone Info.plist. `postinstall` recreates these copies on EAS; the EAS post-install hook checks their hashes. Edit the original files, then run `npm run sync:native`.
...
`.github/workflows/ios-tests.yml` runs three independent macOS jobs on every push to `main`: the tests, the Expo build, and the app-launch smoke test added in round 18. The first compiles and runs the XCTest target against the standalone project generated from `project.yml`, which never touches this Expo project. The second, `build-expo-app`, builds the app that ships, the way EAS builds it: `npm ci`, the native-source mirror check, `npx expo prebuild --platform ios --no-install`, `pod install`, then a signing-disabled simulator build of the generated workspace.

**The workflow is configured to run on every push, and the account's runners have not started since 2026-09-23**, so nothing on `main` after `31b6245` has been built by any of the three jobs. The job definitions and the scripts they call are unchanged and still correct; what is missing is a runner to run them. `AGENT_LOOP.md` has the evidence, the arithmetic and the options.
