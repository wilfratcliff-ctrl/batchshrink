...
| 2 | **Partly fixed in round 28.** One case in `PipelineTests` - where the real `CompressionViewModel` fixture lives - drives the model to `.choosing` and to `.readyToSave` and holds the rule to the model's own `canChoose`, including the assertion this audit's suggested form was missing: that a kept copy is *not* a stage the flow may start over from. `.choosing` and `.readyToSave` are therefore both held now. What remains is the mirror itself: `FlowRoutingTests`' eight cases still feed the rule a hand-written copy of the stage list, so a drift in the list is caught by the two assertions in that one case and not by the eight |
...
| 4 | **Partly fixed in round 28**: `testTheSentencesThatCarryAnInstructionAreTheOnesTheyClaimToBe` pins `.save`'s "Inspect Photos before retrying" and its "extra copy" clause, `.libraryScan`, `.retrieval`, `.cancelled`, and all three `DeletionMode.detail` strings - so the confirmation case's substitution stops being a hole |
...
| 7b | 3 | The audit's own item 7 named a second promise no case holds: "A copy has to be saved, smaller and confirmed in Photos before an original is touched" (`DeletionSheet.swift:27`), which two batch cases drive one half of each | **open**, recorded rather than patched: the sentence is a claim about the *policy* (`DeletionPolicy`'s four inputs), and the case it wants asserts those four together rather than one at a time - which is `DeletionPolicyTests`' business and belongs with them |
