...
| ID | Status |
|----|--------|
| PV1 | **Half fixed in round 27.** The sheet now waits for the item's own status, bounded at 10 seconds of its own, and answers a `.failed` item or a wait that gets nowhere with a sentence instead of a black rectangle. The spinner covers both waits, so the box says "Opening…" until there is something to play and the "press play" note is drawn only then. The device plan's row now states both clocks, because the look's worst case is the fetch's bound and then this one |
| PV2 | **Fixed in round 28.** The one-video preview now has the same three states as the quick look - a spinner while the item decides, the player, and a failure line - through the one `PlayerReadiness.wait` both previews call, with its own sentence ("This copy could not be played, so there is nothing to check. Discard it, then export the video again.") because the failure here means the app's own file is gone rather than that Photos refused anything. The footer that asks the user to check the picture is drawn only once there is something to check |
| PV3 | **Fixed in round 27**: each note is drawn only in the state it describes - "press play" only when there is something that plays, and the iCloud note only when nothing has failed. That last half goes one step past this finding, which said the iCloud sentence was sound in the failure state and should be left where it is: under a line saying a file would not play, a sentence about fetching taking a moment reads as an explanation of something that is not happening. It is a choice, and it is recorded as one |
...
| PV5 | **Fixed in rounds 27 and 28.** Round 27 made the failure's symbol decoration and its sentence the single element, and moved the sentence out of the fixed box so it wraps. Round 28 posts an announcement when a look becomes playable, in both previews, because the spinner's disappearance moves nobody's focus |
| PV6 | **Recorded as an invariant in round 28, not patched.** For every video this app will play, `.current` and `.original` are the same file: `AssetRules` refuses a video with adjustments by name, so no edited asset reaches the grid or a run. `PhotoLibraryService.playerItem` now says that where the option is set, including what has to move if that refusal ever comes out of `AssetRules` - and the reason it is stated rather than changed is that asking for `.original` on an unedited asset would buy nothing and could cost a full iCloud download for a preview |
...
| PV9 | **Fixed for the scrub sheet in round 27**: the load's answer is checked against cancellation before a player is built, because `onDisappear` has already run and will not run again to pause a player created after it. Round 28 gave the one-video preview the same wait through the shared `PlayerReadiness`, so both callers check cancellation before a player exists |
