...
| PV5 | **Half fixed in round 27**: the failure's symbol is decoration and its sentence is the single element, and the sentence that used to sit inside the box now wraps under it. What is still open is the half this finding leads with - nothing posts an announcement when the look *finishes*, so a VoiceOver user hears "Opening…" and then has to go looking |
